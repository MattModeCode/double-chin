"""The Double Chin application server.
...
from double_chin.studio.jobs import JobBusyError, JobManager, JobNotFoundError
...
def create_app(
    ...
    app = FastAPI(title="Double Chin", version=__version__)
    manager = JobManager(
    ...
    async def enroll_voice(
        ...
        with tempfile.TemporaryDirectory(prefix="double-chin-enroll-") as tmp:
            saved: list[Path] = []
            total_bytes = 0
    ...
    def get_audio(job_id: str, download: bool = False):
        ...
        filename = f"double-chin-{job_id}.wav" if download else None
    ...
    def get_doctor():
        ...
        report: dict = {
            "version": __version__,
            "double_chin_home": str(double_chin_home()),
