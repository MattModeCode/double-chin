"""Generation history persistence for Double Chin.

Every completed generation appends one JSON object to
`DOUBLECHIN_HOME/studio/history.jsonl`; the audio itself lives in
`DOUBLECHIN_HOME/studio/jobs/<job_id>/out.wav`. Append-only, newest last on
...
from double_chin.config import ensure_dir, double_chin_home


def studio_dir() -> Path:
    """Return the Studio state directory under the Double Chin home."""
    return double_chin_home() / "studio"
