"""Native desktop shell for ChinAI.

Opens the same local server used by `chinai studio` in a real desktop
...
def _acquire_single_instance_lock() -> object | None:
    """Try to become the sole ChinAI instance; return a handle to keep alive.
    ...
    from chinai.config import chinai_home, ensure_dir

    lock_path = ensure_dir(chinai_home()) / "app.lock"
...
def run() -> int:
    """Launch ChinAI as a native desktop window. Blocks until the window closes."""
    import webview  # deferred: pulls in pyobjc on macOS, only needed for this path

    from chinai.config import migrate_legacy_home
    ...
    if lock is None:
        # Another ChinAI window already owns this session — exit quietly
...
if __name__ == "__main__":
    # Entry point for the bundled ChinAI.app (see packaging/chinai.spec):
