#!/usr/bin/env python3
"""Build a ChinAI speech-style profile from one or more reference recordings.

Transcribes the given audio with word-level timestamps (mlx-whisper), then
derives the statistics `chinai-perform` needs to rewrite a plain script into
...
Run with the skill's own venv, not the system interpreter:
    .claude/skills/chinai-perform/.venv/bin/python scripts/profile_audio.py \
        "~/Downloads/New Recording 63.m4a"

Writes profile.json (machine-readable) and profile.md (human-readable) under
$CHINAI_HOME/style/ (default ~/.chinai/style/).
"""
...
def _chinai_home() -> Path:
    """Reuse ChinAI's own home-dir convention (CHINAI_HOME env, else ~/.chinai)
    so the style profile lives next to enrolled voices without duplicating
    that logic. Falls back to the same default if chinai isn't importable."""
    try:
        ...
        return Path(env_value).expanduser() if env_value else Path.home() / ".chinai"


def render_markdown(profile: Profile) -> str:
    lines = [
        "# ChinAI speech-style profile",
        "",
