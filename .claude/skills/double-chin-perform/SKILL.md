---
name: double-chin-perform
description: Rewrite a plain script into a Double Chin "performance script" that reproduces the user's own pauses, pacing, and speech style. Use when preparing text for Double Chin, making the voice clone sound natural or like the user, or when the user mentions performance scripts, pauses, delivery, or speech style for Double Chin.
---

# Double Chin Perform

Rewrite a plain script into a Double Chin-ready performance script whose
punctuation, line breaks, and inserted fillers reproduce the user's own
pause rhythm and speech style — because Double Chin's engine reads text
...
- The user asks to "prepare," "style," or "perform" a script for Double Chin.
- The user wants Double Chin's output to sound natural, like them, or to keep
...
The profile lives at `$DOUBLECHIN_HOME/style/profile.json` (default
`~/.double-chin/style/profile.json`). If it exists, read it and skip to step 2.
...
.claude/skills/double-chin-perform/.venv/bin/python \
  .claude/skills/double-chin-perform/scripts/profile_audio.py \
...
venv at `.claude/skills/double-chin-perform/.venv` (already provisioned; not the
system Python — Homebrew's Python is externally managed). If that venv is
ever missing, recreate it: `python3 -m venv .claude/skills/double-chin-perform/.venv
&& .claude/skills/double-chin-perform/.venv/bin/pip install mlx-whisper`.
...
| Where the user would naturally pause | Lever to insert | Double Chin result |
|---|---|---|
...
```
# double-chin say --script <this-file> --voice <name> \
...
Tell the user: pauses beyond Double Chin's two fixed silence durations (0.35s /
0.7s) are approximated, not exact — see
[references/levers.md](references/levers.md#the-only-two-silence-durations-that-exist).
If they run `double-chin say --verify`, remind them it scores speaker identity,
...
```
# double-chin say --script launch-update.txt --voice me --exaggeration 0.5 --cfg 0.65 --temperature 0.7
...
- Double Chin's silence model has exactly two fixed durations (0.35s sentence,
  0.7s paragraph) — see
  [references/levers.md](references/levers.md#the-only-two-silence-durations-that-exist).
...
- `--verify` measures speaker identity via cosine similarity against the
  held-out enrolment clip (`src/double_chin/verify.py`) — it does not measure
