...
My mandate was purely adversarial: to refute, weaken, or bound every load-bearing claim Myna makes, so that anything left standing is trustworthy. Conducted 2026-07-11 against branch `build/myna` by reading all source, docs, the installed `chatterbox`/`resemblyzer`/`resemble-perth` packages in `.venv`, and by running the offline unit suite (28 passed). I did not run `myna say`/`enroll` or load the TTS model. Findings are ranked by severity, then mapped back to the seven claims of record.
...
`myna say --verify` scored the output against the exact clip Chatterbox conditioned on — asking "does the output resemble the audio I told the model to imitate," which the generator was directly optimized to satisfy. That is why it lands at 0.9+. Meanwhile the recording kit sold the opposite: "hold out the rest for verification." No holdout existed in code.
...
The watermark claim checks out: `perth.PerthImplicitWatermarker` is applied unconditionally to every generation. But Perth is an imperceptible provenance signal, defeatable by re-encoding, and Myna exposes no detector — it deters no determined misuser. And the actual enrolment floor is `MIN_REFERENCE_SECONDS = 5.0`: the tool clones from 5 seconds, an even lower misuse bar than the 17 s implied elsewhere.
...
- No telemetry or phone-home in `src/`; user audio and enrolled voices stay on disk under `MYNA_HOME`.
...
The core result — "Chatterbox produces a recognizably same-speaker clone, and Myna measures it" — is real. What was oversold, and has now been either fixed or reworded: the *independence* of the measurement, the *coverage* of the tests, the *locality* absolutism, and, most seriously, the *usability of the documented user path*.
